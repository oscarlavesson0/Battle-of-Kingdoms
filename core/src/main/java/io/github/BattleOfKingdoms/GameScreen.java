...
public class GameScreen implements Screen {
    ...
    private static final float GEAR_SIZE = 40f;
    ...
    public void show() {
        ...
        basePopup.setTrainUnitListener(new TrainUnitListener() {
            ...
            @Override public void onCreateBuilding(BaseStats base) { buildingMenuPopup.open(base); }
        });

        statsPopup.setListener(new StatsChosenListener() {
            @Override
            public void onStatsChosen(BaseStats base, int hp, int attack, int speed, int defence, UnitType unitType) {
                CustomUnit unit = baseController.createUnitFromChoice(base, hp, attack, speed, defence, unitType);
                if (unit == null) { statsPopup.ShowError("Not enough gold!"); return; }
                System.out.println("Unit created: HP=" + hp + " ATK=" + attack
                    + " SPD=" + speed + " DEF=" + defence + " TYPE=" + unitType);
            }
        });
        ...
        buildingMenuPopup.setListener(new BuildingChosenListener() {
            ...
            public void OnBuildingChosen(BuildingType buildingType, BaseStats baseStats) {
                ...
                if (building == null) buildingMenuPopup.ShowError("Not enough gold!");
            }
        });
    ...
    private void renderBaseHpBars() {
        for (BaseStats b : new BaseStats[]{ base1, base2 }) {
            ...
            float px   = baseTileCol * WorldMap.TILE_SIZE;
            float py   = baseTileRow * WorldMap.TILE_SIZE;
    ...
    private void renderHud() {
        ...
        hudFont.draw(batch, "Bas P1: " + base1.getCurrentHp() + "/" + base1.getMaxHp(), 20, screenH - 80);
        batch.draw(hpIcon, 170, screenH - 132, 22, 22);
        hudFont.draw(batch, "Bas P2: " + base2.getCurrentHp() + "/" + base2.getMaxHp(), 20, screenH - 110);
